...
if __name__ == "__main__":
	...
	Person_Name = ['Gelasiy', 'Georgiy', 'Mitrofan', 'Valentin', 'John', 'Vyacheslav', 'Ville', 'Denis','Ivan', 'Gleb', 'Dmitrij', 'Nikolay', 'Artem', 'Eduard']
	Person_Patronymic = ['Vsevolodovich', 'Tihonovich', 'Valentinovich', 'Varlamovich', 'Alekseevich', 'Ulianovich', 'Anatolievich', 'Romanovich', 'Ruslanovich', 'Rostislavovich', 'Vyacheslavovich', 'Antonovich', 'Ignatjevich', 'Petrovich']
	Person_Surname = ['Pahomov', 'Shcherbakov', 'Sazonov', 'Zuev', 'Smith', 'Lazarev', 'Kouki', 'Noskov', 'Ustinov', 'Shestakov', 'Turov', 'Komissarov', 'Gordeev', 'Efimov']
	Person_City = ['Omsk', 'Blagoveshchensk', 'Azov', 'Petrozavodsk', 'Moskva', 'Sergiev Posad', 'Belogorsk', 'Kostomuksha']
	
	i = 1;
	while i < 2 :
		new_name = random.sample(Person_Name, 1)[0]
		new_patronymic = random.sample(Person_Patronymic, 1)[0]
		new_surname = random.sample(Person_Surname, 1)[0]
		new_city = random.sample(Person_City, 1)[0]
	
		user = User(uuid.uuid4(), new_name, new_surname, new_patronymic, new_city, Status.PENDING)
